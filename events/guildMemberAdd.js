...
module.exports = async (member) => {
 var kurulus = (Date.now() - member.user.createdTimestamp);
    ...
    if (kurulus > 604800000) {
        member.roles.add(vortex.registration.unregistered);
        member.roles.add(vortex.registration.unregistered);
        member.setNickname(vortex.registration.autonickname);
        member.guild.channels.cache.get(vortex.channels.welcomechannel).send(`:tada: Hey! Sunucumuza hoş geldin ${member}!
        
Hesabın ${kuruluş} tarihinde (**${zaman}**) önce oluşturulmuş.
Sunucumuza kayıt olmak için soldaki ses kanallarından **(V. Confirmed)** birine girmelisin!

Seninle birlikte **${member.guild.memberCount}** kişiye ulaştık.`);
    } else {
        member.setNickname(vortex.registration.susoeciosnickname);
        member.roles.add(vortex.registration.suspecios);
        member.setNickname(vortex.registration.autonickname);
        member.guild.channels.cache.get(vortex.channels.welcomechannel).send({ embeds: [embed.setDescription(`${member}, kullanıcısı sunucuya katıldı hesabı **${zaman2}** önce açıldığı için şüpheli.`)]})
    }
}
...
module.exports.conf = {
    ...
    //SİZE BIRAKIYORUM ZEVKİNİZE GÖRE YAPARSINIZ.
}
